...
export const createNewNotif = async (notifDetails) => {
  if (
    notifDetails.userNotificationsId == "" ||
    notifDetails.NotificationStatus == "" ||
    notifDetails.NotificationContent == "" ||
    notifDetails.NotifyTime == ""
  ) {
...
export const updateNotif = async (user) => {
  ...
  try {
    ...
    const updatedNotif = await API.graphql({
...
export const deleteNotifByMail = async (Mail) => {
  ...
  try {
    const deletedNotif = await API.graphql({
      query: mutations.deleteUserNotifications,
...
export const listNotifbyStatus = async (data) => {
  try {
    const listNotif = await API.graphql({
      ...
      variables: { NotificationStatus: data.NotificationStatus },
    });
